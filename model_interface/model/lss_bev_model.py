...
class LssBevModel(nn.Module):
    def __init__(self, cfg: Configuration):
        ...
        self.frustum = self.create_frustum()    # (depth, h, w, 3)，图像坐标(0~w, 0~y, depth: cfg.d_bound)
    ...
    def create_frustum(self):
        ...
        frustum = torch.stack((x_grid, y_grid, depth_grid), -1)     #x:0~255 H(Pixel)   y:0~255 W(Pixel) z:0.5~12.5 Depth(m)

        return nn.Parameter(frustum, requires_grad=False)

    def get_geometry(self, intrinsics, extrinsics):
        extrinsics = torch.inverse(extrinsics).cuda()   # 相机坐标系（图像pix）到ego坐标系
        rotation, translation = extrinsics[..., :3, :3], extrinsics[..., :3, 3]
        b, n, _ = translation.shape

        points = self.frustum.unsqueeze(0).unsqueeze(0).unsqueeze(-1)   # (1, 1, depth, h, w, 3, 1)
        points = torch.cat((points[:, :, :, :, :, :2] * points[:, :, :, :, :, 2:3],
                            points[:, :, :, :, :, 2:3]), 5)             # (x*z, y*z, z)
        ...
        # （b, n, d, h, w, 3)：图像坐标索引，值是ego坐标系
        return points   #ego坐标系下的 点云坐标

    def encoder_forward(self, images):
        b, n, c, h, w = images.shape
        images = images.view(b * n, c, h, w)
        x, depth = self.cam_encoder(images)     #图像特征
        ...
        x = x.view(b, n, *x.shape[1:])  # (b, n, c, h, w)
        x = x.permute(0, 1, 3, 4, 5, 2) # (b, n, h, w, c)
        return x, depth_prob
    ...
    def proj_bev_feature(self, geom, image_feature):
        ...
        for b in range(batch):
            ...
            geom_b = ((geom_b - (self.bev_start_pos - self.bev_res / 2.0)) / self.bev_res)  # ego坐标量化
            ...
            x_b, geom_b = VoxelsSumming.apply(x_b, geom_b, ranks)   # pooling后的特征跟坐标

            bev_feature = torch.zeros((self.bev_dim[2], self.bev_dim[0], self.bev_dim[1], c),
                                      device=image_feature_b.device)
            bev_feature[geom_b[:, 2], geom_b[:, 0], geom_b[:, 1]] = x_b     # x赋到了rows，y赋到了cols，这个跟ros的坐标系有关，x是竖直轴，y是水平轴，原点在右下角
            tmp_bev_feature = bev_feature.permute((0, 3, 1, 2)).squeeze(0)  # 直接默认z=1了
            output[b] = tmp_bev_feature

        return output

    def calc_bev_feature(self, images, intrinsics, extrinsics):
        geom = self.get_geometry(intrinsics, extrinsics)    # 4个相机坐标系下的 forum深度点转换到 ego坐标系
        x, pred_depth = self.encoder_forward(images)        # 图像特征[Batch, N, D, H, W, C]  D:Depth, N:4
        bev_feature = self.proj_bev_feature(geom, x)        # LSS
