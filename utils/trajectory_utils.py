...
class TrajectoryInfoParser:
    def __init__(self, task_index, task_path):
        ...
        self.trajectory_list = self.make_trajectory()   # 该task下，每帧的pose点位（全局坐标系）
        self.progress_list = self.get_progress_list()   # 1.0 到 0，轨迹路径百分比的倒置
        self.candidate_target_pose = self.get_candidate_target_pose()       #找到和终点yaw角相差较小的traj轨迹点，作为target点位
    ...
    def get_candidate_target_pose(self) -> List[CustomizePose]:
        ...
        for trajectory_item in self.trajectory_list:
            if abs(trajectory_item.yaw - self.trajectory_list[-1].yaw) < 1:  #与终点的yaw比较   TODO: abs BUGS?，只通过yaw角是否会取到中间的轨迹？
    ...
    def get_progress_list(self) -> List[float]:
        distance_list = [0.0]   # 保存每个位置到起始为止的轨迹长度
