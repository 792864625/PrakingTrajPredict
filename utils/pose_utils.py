...
class CustomizePose:
    def __init__(self, x, y, z, roll, yaw, pitch):
        ...
        self.pitch = pitch
    #计算ego坐标系下traj点的位姿
    def get_pose_in_ego(self, world2ego_mat: np.array):             #ego是当前小车坐标系；pose指的是traj 点在 世界坐标系下的位姿；
        pose2world_mat = self.get_homogeneous_transformation().get_matrix()     #
        homogeneous_matrix = world2ego_mat @ pose2world_mat         #这里得到的是pose2ego，也就是ego坐标系下 traj点的位姿态
    ...
    def get_homogeneous_transformation(self):           #将(x,y,z,yaw,pitch,roll转换成矩阵)
...
def pose2customize_pose(pose_info) -> CustomizePose:
