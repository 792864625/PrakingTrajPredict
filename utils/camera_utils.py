...
class CameraInfoParser:
    ...
    def parser_info(self):
        ...
        return intrinsic, extrinsic         # ros ego ->  camera(传统意义上的相机坐标系x朝右,y朝下，z超前)
    ...
    def get_extrinsics(self, x_right, y_right, z_right, yaw_right, pitch_right, roll_right):
        cam2pixel_3 = np.array([
            ...
            [0, 0, -1, 0],
            [1, 0, 0, 0],
            [0, 0, 0, 1]], dtype=float)     #ros->传统意义的相机坐标系
        ...
        veh2cam = cam2pixel_3 @ np.array(np.linalg.inv(cam2veh_3))  # DONE: ros手坐标系到图像坐标系的转换：z轴向上，对应图像中的-y轴；y轴向左，对应图像中的-x轴；x轴冲前，对应图像中深度的z轴
        return veh2cam
...
class ProcessImage:
    def __init__(self, pil_image: Image, intrinsics, extrinsics, target_size=(256, 256)):
        ...
        self.color_jit = torchvision.transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.3)


    def color_aug(self):
        self.pil_image = self.color_jit(self.pil_image)
